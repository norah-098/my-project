...
bool isEmptyStack();
bool isFullStack();
void push(const Typefi newltem);
...
template<class Type>
 void linkedStackType<Type>::push(const Typefi newElement) 
{
...
stackTop = new nodeType<Type>; stackTop->info = current->info; stackTop->link = NULL;
last = stackTop; current = current->link;
while(current != NULL)
{
newNode = new nodeType<Type>; 
...
while(stackTop != NULL)
{
temp = stackTop; stackTop = stackTop ->link; delete temp;
 } 
}
template<class Type>
